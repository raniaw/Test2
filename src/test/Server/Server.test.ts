...
describe('Server test suite', () => {
    test('should create server on port 8888', () => {
        new Server().startServer();
        expect(listenMock.listen).toBeCalledWith(8888);
        expect(responseMock.end).toBeCalled();
    });
    test('should handle login requests', () => {
        requestMock.url = 'http://localhost:8888/login';
        new Server().startServer();
    ...
    test('should handle data requests', () => {
        requestMock.url = 'http://localhost:8888/users'
        new Server().startServer();
